...
class WebsiteMonitor {
	...
	constructor() {
		this.config = {
			...
			statusFile: path.join(
				process.cwd(),
				process.env.STATUS_FILE || "./website-status.json",
			),
		};
	...
	private async loadWebsiteStatuses(): Promise<void> {
		try {
			...
			console.log(
				`Loaded ${statuses.length} website statuses from ${this.config.statusFile}`,
			);
		} catch (error) {
			if ((error as NodeJS.ErrnoException).code === "ENOENT") {
				console.log(
					`Status file ${this.config.statusFile} not found, starting fresh`,
				);
			} else {
	...
	public async saveWebsiteStatuses(): Promise<void> {
		try {
			...
			console.log(
				`Saved ${statuses.length} website statuses to ${this.config.statusFile}`,
			);
		} catch (error) {
